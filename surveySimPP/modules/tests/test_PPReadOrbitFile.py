...
def test_PPReadOrbitFile():
    ...
    expected_first_row = np.array(['S00000t', 0.952105479028, 0.504888475701, 4.899098347472,
                                   148.881068605772, 39.949789586436, 54486.32292808, 54466.0],
                                  dtype=object)

    expected_columns = np.array(['ObjID', 'q', 'e', 'incl', 'node', 'argperi', 't_p', 't_0'],
                                dtype=object)
    ...
    assert e2.value.code == 'ERROR: PPReadOrbitFile: Cannot find ObjID in column headings. Check input and input format.'
