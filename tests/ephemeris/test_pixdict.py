...
from sorcha.ephemeris.simulation_setup import precompute_pointing_information, create_assist_ephemeris, generate_simulations, furnish_spiceypy
...
def test_pixeldict(tmp_path):
	...
	filterpointing = PPReadPointingDatabase(args.pointing_database, configs["observing_filters"], configs["pointing_sql_query"], "lsst")
	...
	assert np.isclose(np.linalg.norm(pixdict.get_observatory_position(54800.0 + 2400000.5) - ecliptic_to_equatorial(r_obs)), 0)
