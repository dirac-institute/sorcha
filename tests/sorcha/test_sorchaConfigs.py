...
from sorcha.utilities.sorchaConfigs import sorchaConfigs, inputConfigs, simulationConfigs, filtersConfigs, saturationConfigs, phasecurvesConfigs, fovConfigs, fadingfunctionConfigs , linkingfilterConfigs, outputConfigs, lightcurveConfigs, activityConfigs, expertConfigs
...
def test_simulationConfigs_int(key_name):
    ...
    assert (error_text.value.code == f"ERROR: expected an int for config parameter {key_name}. Check value in config file.")
...
def test_phasecurveConfigs_inlist(key_name, expected_list):
    ...
    print(f"ERROR: value definitely_fake_bad_key for config parameter {key_name} not recognised. Expecting one of: {expected_list}.")
    assert (
        error_text.value.code
        == f"ERROR: value definitely_fake_bad_key for config parameter {key_name} not recognised. Expecting one of: {expected_list}."
    )
...
def test_fovConfigs_surveyname():
    ...
    with pytest.raises(SystemExit) as error_text:
        test_configs = fovConfigs(**fov_configs)
    assert (error_text.value.code == "ERROR: a default detector footprint is currently only provided for LSST; please provide your own footprint file.")
...
@pytest.mark.parametrize(
    "key_name", ["ssp_detection_efficiency" , "ssp_separation_threshold","ssp_maximum_time","ssp_night_start_utc","ssp_number_observations" , "ssp_number_tracklets","ssp_track_window"])
...
def test_linkingfilter_bounds(key_name):
    ...
    if key_name == "ssp_maximum_time" or key_name == "ssp_track_window":
        ...
        assert error_text.value.code == "ERROR: ssp_detection_efficiency out of bounds (should be between 0 and 1)."
    elif key_name == "ssp_night_start_utc":
        linkingfilter_configs[key_name]= -5
        with pytest.raises(SystemExit) as error_text:
            test_configs = linkingfilterConfigs(**linkingfilter_configs)

        assert error_text.value.code == "ERROR: ssp_night_start_utc must be a valid time between 0 and 24 hours."
...
def test_expert_config_exclusive():
    ...
    assert error_text.value.code == "ERROR: SNR limit and magnitude limit are mutually exclusive. Please delete one or both from config file."
...
def test_expert_config_bool(key_name):
    ...
    assert error_text.value.code == f"ERROR: expected a bool for config parameter {key_name}. Check value in config file."
