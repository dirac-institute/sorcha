...
def PPReadOif(oif_output, inputformat):
    ...
    if not set(padafr.columns.values).issubset(oif_cols):
        pplogger.error(
            "ERROR: PPReadOif: column headings do not match expected OIF column headings. Check format of file."
        )
        sys.exit(
            "ERROR: PPReadOif: column headings do not match expected OIF column headings. Check format of file."
        )
    ...
    with open(filename) as fh:
        for i, line in enumerate(fh):
            ...
            if i > 100:  # because we don't want to scan infinitely - OIF headers are ~30 lines long.
                pplogger.error(
                    "ERROR: PPReadOif: column headings not found. Ensure column headings exist in OIF output and first column is ObjID."
                )
                sys.exit(
                    "ERROR: PPReadOif: column headings not found. Ensure column headings exist in OIF output and first column is ObjID."
                )
